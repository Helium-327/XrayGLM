...
class ImageMixin(BaseMixin):
    ...
    def word_embedding_forward(self, input_ids, output_cross_layer, **kw_args):
        if kw_args["pre_image"] > input_ids.shape[1] or kw_args.get("image", None) is None:
            return self.transformer.word_embeddings(input_ids).contiguous()
        image_emb = self.model(**kw_args).contiguous()
        ...
        pre_txt_emb = self.transformer.word_embeddings(pre_id).contiguous()
        post_txt_emb = self.transformer.word_embeddings(post_id).contiguous()
        return torch.cat([pre_txt_emb, image_emb, post_txt_emb], dim=1).contiguous()
...
class VisualGLMModel(ChatGLMModel):
    def __init__(self, args, transformer=None, **kwargs):
        ...
        # self.parallel_output = kwargs.get("parallel_output", True)
